...
\textbf{The mechanism.} The Formal Interstate Alliance dataset (Gibler 2009) codes a multilateral treaty as a complete dyadic clique: every pair of signatories receives its own bilateral alliance tie, so a single N-member pact contributes C(N,2) alliance dyads and, mechanically, C(N,3) closed triads the moment it enters into force, almost all balanced since the pact itself is entirely positive. Three such instruments dominate the pre-1945 closed-triad population: the \textbf{German Confederation} (Deutscher Bund), a 10-member Austrian- and Prussian-led mutual defense, nonaggression, and consultation pact spanning 1815-1866; the \textbf{Buenos Aires antiwar entente} of 1936, a 21-member pact linking the United States to nearly every Latin American republic; and the \textbf{Nyon Agreement} of 1937, a 9-member Mediterranean anti-piracy defense pact (Britain, France, the Soviet Union, Turkey, Greece, Yugoslavia, Romania, Bulgaria, and Egypt). Together these three instruments account for 86.6\% of all closed triad-years in the 1816-1944 portion of the panel (16,572 of 19,147), and their formation or dissolution moves the year-by-year denominator by hundreds of triads overnight: the near-total absence of variation from 1816 through the 1860s (120 of 121 closed triads in 1850 come from the German Confederation clique alone), the collapse from 94 closed triads in 1866 to 6 in 1867 (the Confederation dissolves after the Austro-Prussian War), and the jump from 39 closed triads in 1935 to 1,371 in 1936 (the Buenos Aires entente entering into force, close to its full theoretical complement of C(21,3)=1,330 new closed triads). None of this is a data error --- each instrument is a real historical treaty, correctly coded --- but the dyadic-clique representation means these are not 120 or 1,330 independent triadic configurations coming into existence; they are one political event, multiplied combinatorially by how the data format represents it.

\textbf{A second pattern: the 1870s-1920s ``gap'' era.} The clique mechanism above explains why the denominator swings wildly, not why the \emph{share} unbalanced is genuinely elevated, not just noisier, for an extended stretch between the two clique eras. Averaged by decade, the share of closed triads unbalanced is near zero both before 1867 (0.1--3.0\%, the German Confederation era) and after 1936 (2.0--4.2\%, the NATO/Warsaw Pact/other 20th-century-bloc era), but runs 7.5--15.0\% throughout the 1870s--1920s, a five-decade window between the two large all-positive cliques, when the diplomatic network was smaller and not yet organized into a dominant stable bloc. In absolute terms this is still a small number of triads (735 unbalanced triad-years across the entire pre-1950 period, against 9,615 from 1950 on) --- a higher share of a much smaller population, not ``many'' unbalanced triads by any standard. Tracing individual years in this window to the underlying triads shows the unbalanced ones are not scattered noise but track known crises: 1876 is dominated by Britain--Austria-Hungary--Russia and Britain--Russia--Ottoman Empire triads, unbalanced amid the Balkan crisis leading to the 1877--78 Russo-Turkish War; 1897 by a cluster of Britain/France/Russia/Ottoman/Greece triads during the Greco-Turkish War; 1904 by Britain--Russia--Japan, unbalanced as the 1902 Anglo-Japanese alliance put Britain against Russia during the Russo-Japanese War; 1907 by a cluster of Central American triads coinciding with the 1906--07 Central American War; and 1919, the single highest point in the figure at 87.5\%, almost entirely from Russia's simultaneous hostility toward Britain, Germany, France, Poland, Japan, and the Baltic states during the Russian Civil War and Allied intervention. The elevated share thus reflects real Great Power crisis periods playing out in a diplomatic network too small and unconsolidated, in that window, to be diluted by a large all-positive bloc the way the periods on either side of it are.

\textbf{Effect on the paper's actual results.} This does not affect the paper's results: the spell-based analysis (Section 3.3) only draws on triads that become \emph{unbalanced}, and a multilateral peace pact is almost entirely positive, essentially never generating an unbalanced triad on its own. Only 40.7\% of pre-1945 unbalanced triad-years touch one of the three big cliques (258 of 634), far smaller than the 86.6\% of the raw denominator; at the level that matters for the analysis, only 115 of 6,277 spells (1.8\%) ever involve a triad overlapping one of the three cliques. Re-running the primary hazard models with these 115 spells excluded leaves the core results essentially unchanged --- embeddedness moves from -2.508 to -2.435 (dissolution) and 0.439 to 0.423 (realignment), both still highly significant (p$<$.001). The mechanism described above is a real feature of the pre-1945 population, not generic ``thin data,'' and poses no threat to the spell-based analysis this paper's conclusions rest on.
...
The alliance-to-hostility/hostility-to-alliance reversal reported in main-text Section 4.3 (Table 4) is not an artifact of the two transition types starting from different triad configurations:

\begin{table*}[t]
\centering
\textbf{Table A11.} Direction-of-change cross-tab: triad configuration at the start of the realignment year, by changed-tie type.
...
Essentially all alliance-to-hostility events (67 of 67) begin, like the hostility-to-alliance events, from the same two-positive-one-negative starting configuration; the tiny residual zero-positive-three-negative configuration (5 events total) contributes only to the hostility-to-alliance side and is too small to explain the reversal. Whatever distinguishes the two tie types, it is not which triad shape they start from.
...
This affects how much confidence the 96.6\% figure deserves, though it does not overturn it. A cluster bootstrap resampling dyad-year transitions (not individual triad-events), 2,000 replications, gives a 95\% interval of [95.2\%, 97.6\%] around the observed 96.6\%, close to the naive triad-level confidence interval in the main text; the hit rate is consistent within as well as between transitions, so this is not a few large transitions dominating the count. Collapsing entirely to one row per unique dyad-year transition (n=241) and asking whether the changed dyad had the maximum unbalanced load in \emph{any} of the triads it resolved gives 80.1\%; testing this against a flat 33.3\% baseline is not appropriate, since the null probability rises with how many triads a transition touches. Recomputing the chance baseline per transition (as $1-(2/3)^n$ for a transition resolving $n$ triads) and averaging across the 241 transitions gives an expected 78.5\% under a no-information null, barely below the observed 80.1\% --- weak evidence on its own; the same-sign comparison below, with a well-defined 50\% null throughout, carries more weight. The pooled 96.6\% figure's qualitative direction holds up, but its \emph{effective} sample size is closer to a few hundred independent political transitions than several thousand triad-level observations, and Table 3's reported N should be read with that in mind.
...
\textbf{The two measures are nearly collinear at this level.} Triadic embeddedness and current unbalanced load correlate at $r=0.967$ across the 10,170 triad-years (nearly identical, $r=0.959$, restricted to the 5,419 realignment-event rows). This is not a definitional identity --- embeddedness counts all overlapping triangles, unbalanced load only the unbalanced subset --- but empirically the \emph{share} of a tie's overlapping triangles that are unbalanced is remarkably stable across the embeddedness distribution: the ratio of current unbalanced load to total embeddedness has a median of 0.320 and an interquartile range of [0.313, 0.324], against a full range of [0, 0.923]. Because the risk set is dominated by ties embedded in a handful of major alliance blocs (Section 4.5), a tie's overlapping triangles come disproportionately from the same few blocs, roughly a third of them unbalanced regardless of bloc size --- so embeddedness and unbalanced load rise together almost one-for-one across triads, even though they need not in principle.
...
\begin{table*}[t]
\centering
\textbf{Table A12.} Realignment hazard with vs.\ without current unbalanced load added to the primary specification.
...
\begin{table*}[t]
\centering
\textbf{Table A13.} Primary hazard models re-estimated under Davis's (1967) weak-balance criterion vs.\ the paper's strong-balance criterion.
